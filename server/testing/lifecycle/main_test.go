...
func readLobbyState(
	...
	require.Equal(t, core.MessageTypeLobbyState, msgType)
...
func requireWinnerGameState(
	...
	require.Equal(t, byte(core.GameEventTypeWinner), data[0])
...
func readUntilWinner(
	...
	for time.Now().Before(deadline) {
		...
		case core.MessageTypeGameState:
			if len(data) > 0 && data[0] == byte(core.GameEventTypeWinner) {
...
func TestSingleplayerLifecycle(t *testing.T) {
...

	{
		...
		err := c.Send(core.MessageTypeStart, nil, defaultTimeout)
		require.NoError(t, err)

		msgType, data := readMessage(t, c, defaultTimeout)
		require.Equal(t, core.MessageTypeReady, msgType)
		require.Equal(t, []byte{0}, data)

		msgType, _ = readMessage(t, c, defaultTimeout)
		require.Equal(t, core.MessageTypeStarted, msgType)
	}

	{
		...
		msgType, data := readMessage(t, c, defaultTimeout)
		require.Equal(t, core.MessageTypeGameState, msgType)
		require.NotEmpty(t, data)

		require.Equal(t, byte(core.GameEventTypeCountdown), data[0])
	}
...

	{
		...
		require.Equal(t, core.MessageTypeSaved, msgType)
...
func TestMultiPlayerLifecycle(t *testing.T) {
...

	{
		...
		require.Equal(t, core.MessageTypePlayerJoined, msgType)
...

	{
		...
		err := c1.Send(core.MessageTypeStart, nil, defaultTimeout)
		require.NoError(t, err)
...

	{
		...
		err := c2.Send(core.MessageTypeStart, nil, defaultTimeout)
		require.NoError(t, err)
...
func TestLobbiesFull(t *testing.T) {
	...
	msgType, _, err := c.Read(defaultTimeout)
	require.NoError(t, err)
	require.Equal(t, core.MessageTypeFull, msgType)
}
...
func TestMultiplayerPlayerDisconnects(t *testing.T) {
...

	{
		...
		for time.Now().Before(deadline) {
			...
			case core.MessageTypePlayerLeft:
				require.Empty(t, data)
				return
